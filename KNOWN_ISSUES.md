...
- Workflow tab changes, same-scope revisits, and Event Signature presentation-only reruns avoid hidden scan/metadata/metric/context work now, but the app can still lag under heavy preview or analysis rendering and needs profiling before broader UI-performance refactors.
...
- UI tab-switch and analysis rendering performance still need profiling for remaining layout/plot costs now that tab switches no longer start hidden scan, metadata rebuild, metric compute, analysis invalidation, or Event Signature data preparation by themselves.
